...
{
    let _reply = Arc::new(reply);
    let handle1 = tokio::spawn(send_reply(first, Arc::clone(&_reply)));
    let handle2 = tokio::spawn(send_reply(second, _reply));
    let _ = tokio::join!(handle1, handle2);
}

async fn send_reply<T>(listener: TcpListener, reply: Arc<T>)
where
    T: Display + Send + Sync + 'static,
    {
        loop {
